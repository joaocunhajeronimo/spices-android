...
Spices is currently only available for iOS

## Prerequisite

You should already be using the `calabash-cucumber` gem and have run the following commands
...
Simply add 'spices' to you Gemfile

```ruby
gem 'spices', '0.0.0', :git => "git://github.com/TitouanVanBelle/spices.git"
```

You now just need to require the gem for example in the `features/support/env.rb` file generated by the `calabash-ios gen`
